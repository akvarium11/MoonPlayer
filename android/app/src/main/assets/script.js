document.addEventListener('DOMContentLoaded', () => {
    // ==========================================
    // 1. PLATFORM DETECTION & SNOWFLAKE BACKGROUND
    // ==========================================
    const isAndroidApp = !!window.AndroidBridge || /Android/i.test(navigator.userAgent);
    const snowContainer = document.getElementById('snow-container');
    const snowflakeCount = isAndroidApp ? 12 : 25;
    const savedSnowPref = localStorage.getItem('moonplayer_snow');
    // On Android/mobile, disable snow by default for silky smooth 60/120 FPS
    let isSnowEnabled = savedSnowPref !== null ? (savedSnowPref === 'true') : !isAndroidApp;
    ...
    function createSnowflake() {
        ...
        snowflake.style.transform = 'translate3d(0, 0, 0)';
        snowflake.style.willChange = 'transform';

        // Only apply blur on desktop with high performance GPU
        if (!isAndroidApp && window.innerWidth > 900) {
    ...
    if (window.AndroidBridge || ('ontouchstart' in window && window.innerWidth <= 900)) {
        isTiltEnabled = false;
        isGlowEnabled = false;
        document.body.classList.add('is-android');
    }
    let accentColor = localStorage.getItem('moonplayer_accent') || '#ffffff';
    let cachedAccentColor = accentColor;
    ...
    document.addEventListener('mousemove', (e) => {
        if (!playerCard || isAndroidApp || window.innerWidth <= 900) return;
    ...
    // Screen dimensions helper for stable mobile background
    function updateFixedScreenDimensions() {
        const stableHeight = Math.max(window.screen?.height || 0, window.screen?.availHeight || 0, window.innerHeight || 0);
        const stableWidth = Math.max(window.screen?.width || 0, window.screen?.availWidth || 0, window.innerWidth || 0);
        if (stableHeight > 0) document.documentElement.style.setProperty('--app-fixed-height', `${stableHeight}px`);
        if (stableWidth > 0) document.documentElement.style.setProperty('--app-fixed-width', `${stableWidth}px`);
    }
    updateFixedScreenDimensions();

    window.addEventListener('orientationchange', () => {
        setTimeout(updateFixedScreenDimensions, 150);
    });

    let lastWinWidth = window.innerWidth;
    window.addEventListener('resize', () => {
        if (Math.abs(window.innerWidth - lastWinWidth) > 50) {
            lastWinWidth = window.innerWidth;
            updateFixedScreenDimensions();
        }
        if ((isAndroidApp || window.innerWidth <= 900) && playerCard) {
    ...
    if (mainContainer) {
        if (window.AndroidBridge || window.innerWidth <= 900) {
            mainContainer.classList.remove('tilt-enabled');
        } else {
    ...
    function isSoundCloudPath(p) {
        if (!p || typeof p !== 'string') return false;
        const lp = p.toLowerCase();
        return lp.startsWith('soundcloud:') || lp.startsWith('sc:') || /^\d+$/.test(p) || lp.includes('soundcloud.com') || lp.includes('/api/soundcloud/');
    }

    function extractSoundCloudId(p) {
        if (!p || typeof p !== 'string') return null;
        if (p.startsWith('soundcloud:')) return p.replace('soundcloud:', '');
        if (p.startsWith('sc:')) return p.replace('sc:', '');
        if (/^\d+$/.test(p)) return p;
        const streamMatch = p.match(/(?:tracks\/|stream\/)(\d+)/);
        if (streamMatch) return streamMatch[1];
        return null;
    }

    function cleanTrackForCache(t) {
        if (!t) return null;
        const scId = t.id || (t.path ? String(t.path).replace(/^soundcloud:/, '') : '');
        return {
            id: scId,
            title: t.title || 'Untitled Track',
            artist: t.artist || 'SoundCloud Artist',
            album: t.album || 'SoundCloud',
            albumKey: t.albumKey || `sc_track_${scId}`,
            cover: t.cover || DEFAULT_COVER,
            duration: typeof t.duration === 'number' ? t.duration : 0,
            path: t.path || `soundcloud:${scId}`,
            src: t.src || `/api/soundcloud/stream/${scId}`,
            format: 'soundcloud',
            isSoundCloud: true
        };
    }
    ...
    function loadSoundCloudCache() {
        try {
            ...
            if (stored) {
                ...
                list.forEach(t => {
                    if (t && (t.path || t.id)) {
                        const pathKey = t.path || `soundcloud:${t.id}`;
                        t.path = pathKey;
                        soundCloudTracksCache.set(pathKey, t);
                        if (t.id) soundCloudTracksCache.set(`soundcloud:${t.id}`, t);
                    }
                });
            }
        } catch (e) {
            console.warn("Failed to load SoundCloud cache:", e);
        }
    }
    ...
    function saveSoundCloudCache() {
        try {
            ...
            const entries = Array.from(soundCloudTracksCache.values());
            for (let i = entries.length - 1; i >= 0 && cleaned.length < 300; i--) {
                const c = cleanTrackForCache(entries[i]);
                if (c && c.path && !seenPaths.has(c.path)) {
                    seenPaths.add(c.path);
                    cleaned.unshift(c);
                }
            }
            localStorage.setItem('moonplayer_sc_cache', JSON.stringify(cleaned));
        } catch (e) {
            console.warn("Failed to save SoundCloud cache to localStorage:", e);
            try {
                const smaller = Array.from(soundCloudTracksCache.values()).slice(-80).map(cleanTrackForCache).filter(Boolean);
                localStorage.setItem('moonplayer_sc_cache', JSON.stringify(smaller));
            } catch (e2) {}
        }
    }
    ...
    function registerSoundCloudTrack(track) {
        ...
        if (!track.id && track.path && track.path.startsWith('soundcloud:')) {
            track.id = track.path.replace('soundcloud:', '');
        }
        ...
        if (!track.album) track.album = 'SoundCloud';
        if (!track.cover) track.cover = DEFAULT_COVER;
        track.format = 'soundcloud';
        track.isSoundCloud = true;

        soundCloudTracksCache.set(track.path, track);
        if (track.id) {
            soundCloudTracksCache.set(`soundcloud:${track.id}`, track);
        }
        saveSoundCloudCache();

        let existing = allSongs.find(s => s.path === track.path || (track.id && s.id && String(s.id) === String(track.id)));
        if (existing) {
    ...
    // Load cached SoundCloud tracks on startup and populate allSongs
    loadSoundCloudCache();
    ...
    function initAudioContext() {
        ...
        try {
            ...
            // Start Visualizer loop if active/expanded
    ...
    function resizeCanvas() {
        ...
        const targetWidth = Math.floor(canvas.parentElement.clientWidth * (window.devicePixelRatio || 1));
        const targetHeight = Math.floor(canvas.parentElement.clientHeight * (window.devicePixelRatio || 1));
    ...
    function drawVisualizer() {
        ...
        const minInterval = (isAndroidApp || window.innerWidth <= 900) ? 33 : 16;
        if (now - lastVisualizerDrawTime < minInterval) {
            return;
        }
        lastVisualizerDrawTime = now;
        ...
        ctx.fillStyle = cachedAccentColor || '#ffffff';
    ...
    function selectPlaylist(playlistName) {
        ...
        const pendingScFetches = [];

        const playlistSongs = savedPaths.map(p => {
            // 1. Direct match in songPool
            let found = songPool.find(s => s.path === p || (s.id && (`soundcloud:${s.id}` === p || String(s.id) === p)));
            
            // 2. Look in SoundCloud cache
            if (!found) {
                if (soundCloudTracksCache.has(p)) {
                    found = soundCloudTracksCache.get(p);
                } else if (isSoundCloudPath(p)) {
                    const scId = extractSoundCloudId(p);
                    if (scId) {
                        found = soundCloudTracksCache.get(`soundcloud:${scId}`) || soundCloudTracksCache.get(scId);
                    }
                }
            }

            // 3. If SoundCloud track not yet in cache: create instant fallback so track is NEVER dropped!
            if (!found && isSoundCloudPath(p)) {
                const scId = extractSoundCloudId(p);
                if (scId) {
                    found = {
                        id: scId,
                        path: `soundcloud:${scId}`,
                        src: `/api/soundcloud/stream/${scId}`,
                        url: `/api/soundcloud/stream/${scId}`,
                        title: `SoundCloud Track #${scId}`,
                        artist: 'SoundCloud',
                        album: 'SoundCloud',
                        albumKey: `sc_track_${scId}`,
                        cover: DEFAULT_COVER,
                        duration: 0,
                        format: 'soundcloud',
                        isSoundCloud: true,
                    ...
                    registerSoundCloudTrack(found);
                    pendingScFetches.push(scId);
                }
            }

            if (found) {
                if (!allSongs.some(s => s.path === found.path)) {
                    found.globalIndex = allSongs.length;
                    allSongs.push(found);
                } else if (found.globalIndex === undefined) {
                    const existing = allSongs.find(s => s.path === found.path);
                    found.globalIndex = existing.globalIndex !== undefined ? existing.globalIndex : allSongs.indexOf(existing);
                }
            }
            return found;
        }).filter(Boolean);
        ...
        if (pendingScFetches.length > 0) {
            fetchMissingSoundCloudTracks(pendingScFetches, playlistName);
        }
    ...
    async function fetchMissingSoundCloudTracks(ids, playlistName) {
        ...
        for (let i = 0; i < uniqueIds.length; i += concurrency) {
            ...
            await Promise.all(chunk.map(async (id) => {
                try {
                    const res = await fetch(`/api/soundcloud/track/${id}`);
                    if (!res.ok) return;
                    const data = await res.json();
                    if (data && data.success && data.track) {
                        const track = data.track;
                        registerSoundCloudTrack(track);
                        hasUpdates = true;

                        const inAll = allSongs.find(s => s.path === track.path || (s.id && String(s.id) === String(track.id)));
                        if (inAll) {
                            inAll.title = track.title;
                            inAll.artist = track.artist;
                            inAll.album = track.album;
                            inAll.cover = track.cover;
                            inAll.duration = track.duration;
                            inAll.isLoadingMetadata = false;
                        }

                        if (currentSelectedPlaylistName === playlistName && currentPlaylist) {
                            const inCurrent = currentPlaylist.find(s => s.path === track.path || (s.id && String(s.id) === String(track.id)));
                            if (inCurrent) {
                                inCurrent.title = track.title;
                                inCurrent.artist = track.artist;
                                inCurrent.album = track.album;
                                inCurrent.cover = track.cover;
                                inCurrent.duration = track.duration;
                                inCurrent.isLoadingMetadata = false;
                            }
                        }
                    }
                } catch (err) {
                    console.warn(`[SoundCloud] Failed to fetch track ${id}:`, err);
                }
            }));
        }
        ...
        if (hasUpdates) {
            saveSoundCloudCache();
            if (currentSelectedPlaylistName === playlistName) {
                updateCurrentPlaylistDOM();
            }
        }
    }

    function updateCurrentPlaylistDOM() {
        if (!currentPlaylist) return;
        const items = document.querySelectorAll('.song-item[data-global-index]');
        items.forEach(el => {
            const gIdx = parseInt(el.getAttribute('data-global-index'), 10);
            const song = allSongs[gIdx];
            if (song) {
                const titleTextEl = el.querySelector('.song-title-text');
                if (titleTextEl && titleTextEl.textContent !== song.title) {
                    titleTextEl.textContent = song.title;
                }
                const artistEl = el.querySelector('.song-artist');
                if (artistEl) {
                    artistEl.innerHTML = formatArtistLinks(song.artist);
                }
                const albumEl = el.querySelector('.song-album');
                if (albumEl) {
                    albumEl.textContent = song.album || '';
                }
                const durEl = el.querySelector('.song-duration');
                if (durEl && song.duration > 0) {
                    durEl.textContent = formatTime(song.duration);
                }
            }
        });

        if (detailCover && (detailCover.src.includes('default') || detailCover.src.includes('icon.png') || !detailCover.src)) {
            if (currentPlaylist.length > 0 && currentPlaylist[0].cover && !currentPlaylist[0].cover.includes('default')) {
                detailCover.src = currentPlaylist[0].cover;
            }
        }
    }
    ...
    function showTrackContextMenu(e, song) {
        ...
        // Remove from current playlist option (if viewing a playlist)
        if (currentSelectedPlaylistName !== null) {
            const removeFromPlaylistOption = document.createElement('div');
            removeFromPlaylistOption.className = 'playlist-context-menu-item';
            removeFromPlaylistOption.innerHTML = `
                <i class="fa-solid fa-trash-can" style="color: #ef4444;"></i>
                <span style="color: #ef4444;">Remove from this playlist</span>
            `;
            removeFromPlaylistOption.addEventListener('click', () => {
                menu.remove();
                const pathToRemove = song.path;
                ...
                renderLibraryPanel();
                showToast(`Removed "${song.title}" from "${currentSelectedPlaylistName}"`);
            });
            menu.appendChild(removeFromPlaylistOption);
        ...
        if (x === undefined || y === undefined) {
            if (e && e.target && typeof e.target.getBoundingClientRect === 'function') {
                ...
                y = Math.max(10, (window.innerHeight - 200) / 2);
            }
        }
        const menuWidth = 200;
            ...
            if (searchType === 'album') {
                ...
                div.innerHTML = `
                    ...
                    <div class="result-actions">
                        ...
                        <button class="result-action-btn more-btn" title="More options">
                ...
                if (playlistTracks.length > 0) {
                    const firstPath = playlistTracks[0];
                    firstSong = allSongs.find(s => s.path === firstPath || (s.id && (`soundcloud:${s.id}` === firstPath || String(s.id) === firstPath)));
                    if (!firstSong && soundCloudTracksCache.has(firstPath)) {
                        firstSong = soundCloudTracksCache.get(firstPath);
                    } else if (!firstSong && isSoundCloudPath(firstPath)) {
                        const scId = extractSoundCloudId(firstPath);
                        if (scId) firstSong = soundCloudTracksCache.get(`soundcloud:${scId}`);
                    }
                }
        ...
        artistAlbums.forEach((album, idx) => {
            ...
            card.innerHTML = `
                <div class="grid-album-cover-wrapper">
                    <img src="${album.cover || DEFAULT_COVER}" class="grid-album-cover" alt="${escapeHtml(album.title)}" onerror="this.onerror=null;this.src='${DEFAULT_COVER}';">
                </div>
    ...
    function renderPlaylistView(title, artist, metaText, coverUrl) {
        if (currentDescriptionAbortController) {
            currentDescriptionAbortController.abort();
            currentDescriptionAbortController = null;
        }
        if (detailDescription) {
            detailDescription.classList.add('hidden');
            detailDescription.innerHTML = '';
        ...
        if (typeof window.setMobileView === 'function' && (window.innerWidth <= 900 || window.AndroidBridge)) {
            window.setMobileView('details');
        }
        const selectedDetailsView = document.getElementById('selected-details-view');
        if (selectedDetailsView) {
            selectedDetailsView.style.animation = 'none';
        ...
        const displayTitle = (title && title.length > maxTitleLen) ? title.slice(0, maxTitleLen).trim() + '...' : (title || '');

        if (currentSelectedPlaylistName && title === currentSelectedPlaylistName) {
            detailTitle.innerHTML = `
                <span title="${escapeHtml(title)}">${escapeHtml(displayTitle)}</span>
        ...
        detailCover.onerror = function() { this.onerror = null; this.src = DEFAULT_COVER; };
    ...
    function playTrack(target, animateDirection = 0, forceReload = false) {
        ...
        if (miniCoverEl) {
            miniCoverEl.onerror = function() { this.onerror = null; this.src = DEFAULT_COVER; };
            miniCoverEl.src = song.cover || DEFAULT_COVER;
        }
        ...
        if (animateDirection !== 0 && coversContainer) {
            ...
            newImg.onerror = function() { this.onerror = null; this.src = DEFAULT_COVER; };
            ...
            const trackCover = document.getElementById('track-cover');
            if (trackCover) {
                trackCover.onerror = function() { this.onerror = null; this.src = DEFAULT_COVER; };
    ...
    function updateLyricsDisplay(currentTime) {
        if (!currentLyrics || currentLyrics.length === 0 || !dynamicIsland || !dynamicIsland.classList.contains('lyrics-expanded')) return;
    ...
    async function syncSoundCloudStatus() {
        ...
        const disconnectBtn = document.getElementById('soundcloud-disconnect-btn');
        ...
        try {
            ...
            console.error('SoundCloud sync error:', e);
            if (statusDot) statusDot.className = 'soundcloud-status-dot error';
            if (statusMsg) statusMsg.textContent = 'Offline / server unavailable';
            if (disconnectBtn) disconnectBtn.classList.add('hidden');
        }
    }
    ...
    async function openSoundCloudPlaylist(playlist) {
        ...
        try {
            ...
            const rawDesc = (plData.description || '').trim();
            if (rawDesc) {
                renderDescriptionContent(rawDesc, [], null, null, null);
    ...
    async function openSoundCloudAlbum(album) {
        ...
        try {
            ...
            const rawDesc = (albData.description || '').trim();
            if (rawDesc) {
                renderDescriptionContent(rawDesc, [], null, null, null);
            } else if (detailDescription) {
                detailDescription.classList.add('hidden');
    ...
    if (soundCloudDisconnectBtn) {
        soundCloudDisconnectBtn.addEventListener('click', async (e) => {
            e.stopPropagation();
            soundCloudDisconnectBtn.disabled = true;
            try {
                await fetch('/api/soundcloud/config', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    // Helper: Resize massive camera images and return a detached Blob for reliable IndexedDB storage
            ...
            img.onload = () => {
                ...
                if (w > maxDim || h > maxDim) {
                    ...
                    file.arrayBuffer().then((buffer) => {
                        resolve(new Blob([buffer], { type: file.type || 'image/jpeg' }));
                    }).catch(() => resolve(file));
                }
            };
            img.onerror = () => {
    ...
    if (bgFileInput) {
        bgFileInput.addEventListener('change', async (e) => {
            ...
            try {
                ...
                const processedBlob = await processBgImageFile(file);
                await saveBgImage(processedBlob);

                if (currentBgObjectURL) URL.revokeObjectURL(currentBgObjectURL);
                currentBgObjectURL = URL.createObjectURL(processedBlob);
    ...
    function updateMediaSessionMetadata(song) {
        if (!song) return;
        window._currentPlayingSong = song;
        ...
        if (typeof window.syncAndroidPlayback === 'function') {
            window.syncAndroidPlayback(song);
        }
    }
    ...
    function updateMediaSessionPlaybackState(state) {
        ...
        if (typeof window.syncAndroidPlayback === 'function') {
            window.syncAndroidPlayback(null, state);
        }
    }
    ...
    // Global track getters for AndroidBridge and MediaSession synchronization
    window.getCurrentTrack = function() {
        if (typeof allSongs !== 'undefined' && typeof currentlyPlayingIndex !== 'undefined' && currentlyPlayingIndex >= 0 && allSongs[currentlyPlayingIndex]) {
            return allSongs[currentlyPlayingIndex];
        }
        return window._currentPlayingSong || null;
    };

    window.isCurrentTrackLiked = function(track = null) {
        const current = track || (window.getCurrentTrack ? window.getCurrentTrack() : null);
        if (!current) return false;
        const likeBtn = document.getElementById('like-btn');
        if (likeBtn && (likeBtn.classList.contains('active') || likeBtn.querySelector('.fa-solid.fa-heart:not(.fa-regular)'))) {
            return true;
        }
        if (typeof isLiked === 'function') return isLiked(current);
        if (typeof likedSongs !== 'undefined' && Array.isArray(likedSongs)) {
            return likedSongs.some(s => (s.id && s.id === current.id) || (s.path && s.path === current.path));
        }
        return false;
    };
});
...
// ==========================================
// ANDROID INTEGRATION & MOBILE VIEWPORT
// ==========================================
(function() {
    const tabLib = document.getElementById('tab-btn-library');
    const tabDet = document.getElementById('tab-btn-details');
    ...
    window.setMobileView = function(view) {
        if (!dashboard) return;
        if (view === 'details') {
            dashboard.classList.add('show-details-view');
            if (tabLib) tabLib.classList.remove('active');
            if (tabDet) tabDet.classList.add('active');
            if (window.AndroidBridge && window.AndroidBridge.setViewState) {
                window.AndroidBridge.setViewState('details');
            }
        } else {
            dashboard.classList.remove('show-details-view');
            if (tabLib) tabLib.classList.add('active');
            if (tabDet) tabDet.classList.remove('active');
            if (window.AndroidBridge && window.AndroidBridge.setViewState) {
                window.AndroidBridge.setViewState('library');
            }
        }

        // Auto-collapse Dynamic Island if expanded and not pinned on tab switch
        const island = document.getElementById('dynamic-island');
        const islandLockBtn = document.getElementById('island-lock-btn');
        const isLocked = islandLockBtn && islandLockBtn.classList.contains('pinned');
        if (island && island.classList.contains('expanded') && !isLocked) {
            island.classList.remove('expanded', 'locked', 'queue-expanded', 'lyrics-expanded');
            if (typeof stopVisualizer === 'function') stopVisualizer();
        }
    };

    if (tabLib) tabLib.addEventListener('click', () => setMobileView('library'));
    if (tabDet) tabDet.addEventListener('click', () => setMobileView('details'));
    if (navBackBtn) navBackBtn.addEventListener('click', () => setMobileView('library'));

    // Switch to details view on clicking any track or album item
    document.addEventListener('click', (e) => {
        const item = e.target.closest('.result-item, .album-card, .artist-card, .playlist-card');
        if (item && (window.innerWidth <= 900 || window.AndroidBridge)) {
            setMobileView('details');
        }
    });

    // Rescan Device Music Library buttons
    const rescanBtn1 = document.getElementById('refresh-library-btn');
    const rescanBtn2 = document.getElementById('rescan-library-settings-btn');
    const handleRescan = () => {
        if (window.AndroidBridge && window.AndroidBridge.requestStoragePermission) {
    ...
    if (rescanBtn2) rescanBtn2.addEventListener('click', handleRescan);

    // Android OS MediaSession & Notification Sync
    const bgAudioEl = document.getElementById('bg-audio');
    let lastSyncTime = 0;

    function syncAndroidPlayback(customSong = null, forcedState = null) {
        if (!window.AndroidBridge || !bgAudioEl) return;
        const now = Date.now();
        if (!customSong && forcedState === null && now - lastSyncTime < 300) return;
        lastSyncTime = now;

        const current = customSong || (window.getCurrentTrack ? window.getCurrentTrack() : null) || window._currentPlayingSong;
        if (!current) return;

        const isPlaying = (forcedState !== null) ? (forcedState === 'playing') : (!bgAudioEl.paused);
        const title = current.title || current.name || 'MoonPlayer';
        const artist = current.artist || 'Unknown Artist';
        const album = current.album || '';
        const cover = current.folderCoverUrl || current.cover || '/assets/icon.png';
        const isLikedTrack = window.isCurrentTrackLiked ? window.isCurrentTrackLiked(current) : false;
        const positionMs = Math.floor((bgAudioEl.currentTime || 0) * 1000);
        const durationSec = (bgAudioEl.duration && !isNaN(bgAudioEl.duration) && bgAudioEl.duration > 0)
            ? bgAudioEl.duration
            : (current.duration || 0);
    ...
    window.syncAndroidPlayback = syncAndroidPlayback;
    ...
    const likeBtnGlobal = document.getElementById('like-btn');
    if (likeBtnGlobal) {
        likeBtnGlobal.addEventListener('click', () => {
            setTimeout(() => syncAndroidPlayback(), 150);
        });
    }

    // Register AndroidBridgeCallbacks
    window.AndroidBridgeCallbacks = {
        onPlay: () => {
            const playBtn = document.getElementById('play-btn');
            const bgAudio = document.getElementById('bg-audio');
            if (bgAudio && bgAudio.paused && playBtn) {
                playBtn.click();
            }
        },
        onPause: () => {
        ...
        onSeekTo: (sec) => {
            const bgAudio = document.getElementById('bg-audio');
            if (bgAudio && Number.isFinite(sec)) {
                bgAudio.currentTime = sec;
                syncAndroidPlayback();
            }
        },
        onLike: () => {
        ...
        onBackPressed: () => {
            ...
            if (eqModal && eqModal.classList.contains('visible')) {
                eqModal.classList.remove('visible');
                return true;
            }
            const settingsModal = document.getElementById('settings-modal');
    ...
    // Pause on Zero Volume Setting
    const togglePauseOnZeroVol = document.getElementById('toggle-pause-on-zero-volume');
    if (togglePauseOnZeroVol) {
        const saved = localStorage.getItem('pause_on_zero_volume') === 'true';
        togglePauseOnZeroVol.checked = saved;
        if (window.AndroidBridge && window.AndroidBridge.setPauseOnZeroVolume) {
            window.AndroidBridge.setPauseOnZeroVolume(saved);
        }
        togglePauseOnZeroVol.addEventListener('change', (e) => {
            const enabled = e.target.checked;
            localStorage.setItem('pause_on_zero_volume', enabled);
            if (window.AndroidBridge && window.AndroidBridge.setPauseOnZeroVolume) {
                window.AndroidBridge.setPauseOnZeroVolume(enabled);
            }
            if (typeof showToast === 'function') {
                showToast(enabled ? 'Pause on zero volume enabled' : 'Pause on zero volume disabled');
            }
        });
    }

    let wasPausedByZeroVolumeWeb = false;
    if (bgAudioEl) {
        bgAudioEl.addEventListener('volumechange', () => {
            if (localStorage.getItem('pause_on_zero_volume') !== 'true') return;
            if (bgAudioEl.volume === 0 || bgAudioEl.muted) {
                if (!bgAudioEl.paused) {
                    wasPausedByZeroVolumeWeb = true;
                    bgAudioEl.pause();
                }
            } else if (wasPausedByZeroVolumeWeb) {
                wasPausedByZeroVolumeWeb = false;
                bgAudioEl.play().catch(() => {});
            }
        });
    }

    // Auto trigger library scan
    setTimeout(() => {
        if (typeof reloadLibraryFromServer === 'function') {
            reloadLibraryFromServer();
        }
